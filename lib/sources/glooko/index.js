...
function glookoSource (opts, axios) {
  ...
  function generate_driver (builder) {
    builder.support_session({
      ...
      delays: {
        // Proactively expire the session every 6 hours to force re-login,
        // as Glooko sessions can silently go stale.
        REFRESH_AFTER_SESSSION_DELAY: 6 * 60 * 60 * 1000,
        EXPIRE_SESSION_DELAY: 6 * 60 * 60 * 1000,
      }
    });
