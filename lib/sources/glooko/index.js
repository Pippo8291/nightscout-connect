...
function glookoSource (opts, axios) {
  ...
  function generate_driver (builder) {
    builder.support_session({
      ...
      delays: {
        // Do not proactively expire the session on a timer — only re-login
        // when the server actually rejects the session (fetch error → SESSION_REQUIRED).
        // Use 32-bit-safe max (~24.8 days); Number.MAX_SAFE_INTEGER overflows setTimeout.
        REFRESH_AFTER_SESSSION_DELAY: 2147483647,
        EXPIRE_SESSION_DELAY: 2147483647,
      }
    });
