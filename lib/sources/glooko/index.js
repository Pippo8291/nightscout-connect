...
// Times (local clock) at which the Glooko session should be renewed.
// On the first fetch cycle after any of these times, the session is expired
// and re-authenticated so data is always fresh.
const RENEWAL_TIMES_HM = [
  { hours: 8,  minutes: 30 },
  { hours: 12, minutes: 30 },
  { hours: 18, minutes: 30 },
  { hours: 23, minutes: 59 },
...
// Returns milliseconds from now until the next scheduled renewal time,
// evaluated in the timezone described by timezoneOffsetMs.
// timezoneOffsetMs is stored negated (UTC+1 → -3600000), so
//   localNow = new Date(Date.now() - timezoneOffsetMs)
function msUntilNextRenewal(timezoneOffsetMs) {
  const now = Date.now();
  const localNow = new Date(now - timezoneOffsetMs);

  // Midnight of today in local time, expressed as a UTC epoch ms value
  ...
  for (const { hours, minutes } of RENEWAL_TIMES_HM) {
    // Renewal moment in local-clock ms since epoch (still UTC-based arithmetic)
    const renewalLocalMs = todayMidnightMs + (hours * 60 + minutes) * 60 * 1000;
    // Convert back to UTC wall-clock for comparison
    const renewalUtcMs = renewalLocalMs + timezoneOffsetMs;
    if (renewalUtcMs > now) {
      return renewalUtcMs - now;
    }
  }

  // All today's slots have passed — use the first slot tomorrow
  const tomorrowMidnightMs = todayMidnightMs + 24 * 60 * 60 * 1000;
  const first = RENEWAL_TIMES_HM[0];
  const nextUtcMs = tomorrowMidnightMs + (first.hours * 60 + first.minutes) * 60 * 1000 + timezoneOffsetMs;
...
function glookoSource (opts, axios) {
  ...
  // Bare fetch() has no default timeout: a connect that never completes hangs
  // the frame instead of failing it, so nothing retries.
  var GLOOKO_TIMEOUT_MS = 30 * 1000;
  function glooko_timeout ( ) {
    return AbortSignal.timeout(GLOOKO_TIMEOUT_MS);
  }
  var impl = {
    authFromCredentials ( ) {
      return fetch("https://de-fr.my.glooko.com/users/sign_in", { method: "GET", redirect: "follow", signal: glooko_timeout( ) })
          ...
          .catch((error) => {
            ...
            // Swallowing this resolved the session as `undefined`, so the
            // machine carried on with no cookies instead of retrying.
            throw error;
          });
    },
  ...
  function generate_driver (builder) {
    builder.support_session({
      ...
      delays: {
        // Expire the session at the next scheduled renewal time (local clock:
        // 08:30, 12:30, 18:30, 23:59) so data is always refreshed at known
        // boundaries.  The delay is re-evaluated each time Active is entered,
        // so it tracks wall-clock time even across day boundaries.
        REFRESH_AFTER_SESSSION_DELAY: () => msUntilNextRenewal(opts.glookoTimezoneOffset),
        EXPIRE_SESSION_DELAY: () => msUntilNextRenewal(opts.glookoTimezoneOffset),
      }
    });
