...
, v3API: '/api/v3/graph/data?patient=_PATIENT_&startDate=_STARTDATE_&endDate=_ENDDATE_&series[]=deliveredBolus&series[]=pumpOp5ManualMode&series[]=interruptedBolus&series[]=bgHighManual&series[]=bgLowManual&series[]=bgNormalManual&series[]=cgmHigh&series[]=cgmLow&series[]=cgmNormal&series[]=setSiteChange&locale=en-GB'
  ...
  var impl = {
    authFromCredentials ( ) {
      return fetch("https://de-fr.my.glooko.com/users/sign_in", { method: "GET", redirect: "follow" })
          ...
          .then(({ csrfToken, firstCookie }) => {
            ...
            return fetch("https://de-fr.my.glooko.com/users/sign_in?id=login_form&locale=en", requestOptions)
                .then((response) => {
                  ...
                  return fetch("https://de-fr.my.glooko.com/api/v3/session/users", requestOptions)
                      .then((response) => response.json().then((userData) => {
                        ...
                        return { cookies: authCookie, user: { userLogin: userData.currentPatient }};
                      }));
                });
          })
          .catch((error) => {
    ...
    dataFromSesssion (session, last_known) {
     ...
     const requestOptions = {
      ...
      //     + ('0' + (Math.max(1,myDate.getDate()-1))).slice(-2);
      ...
      return Promise.all([
        ...
        var result = results[0].series
         var some = {
            //food: results[0].foods,
            //insulins: results[1].insulins,
            //scheduledBasals: results[0].scheduledBasals,
            normalBoluses: result.deliveredBolus,
            interruptedBoluses: result.interruptedBolus,
            cgmReadings: [...result.cgmHigh, ...result.cgmNormal,...result.cgmLow],
            bgManuals: [...result.bgHighManual, ...result.bgNormalManual,...result.bgLowManual],
           setSiteChanges: [...new Set(result.setSiteChange)],
           manualModes: [...result.pumpOp5ManualMode]
