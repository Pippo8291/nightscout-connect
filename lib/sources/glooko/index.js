...
const glookoSessionManager = require('./glooko_session_manager');
  ...
  var impl = {
    ...
    dataFromSesssion (session, last_known) {
      ...
      function fetcher (endpoint) {
        ...
        const myHeaders = new Headers();
        myHeaders.append("Cookie", glookoSessionManager?.getSession().cookies);
     ...
     const requestOptions = {
      ...
      let v3APIURL = Defaults.v3API.replace('_PATIENT_',glookoSessionManager?.getSession().user.userLogin?.glookoCode).replace('_STARTDATE_', dateStringStart + "T00:00:00.000Z").replace('_ENDDATE_', dateString + 'T23:59:59.999Z');
      let v3PumpSettings = Defaults.v3PumpSettings.replace('_PATIENT_',glookoSessionManager?.getSession().user.userLogin?.glookoCode);
      ...
      function constructUrl(endpoint) {
        ...
        const url = endpoint + "?patient=" + glookoSessionManager?.getSession().user.userLogin.glookoCode
    ...
    transformData (batch) {
      ...
      console.log("TODO TRANSFORM", batch.length);
