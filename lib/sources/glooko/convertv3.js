...
// Each hypoprotect segment arrives as 5 polygon points sharing the same timestamp.
// This deduplicates to one entry per segment then merges consecutive segments
// (gap <= 1 second) into a single contiguous period.
function mergeHypoSegments(rawSegments) {
  const segMap = new Map();
  for (const pt of rawSegments) {
    if (!segMap.has(pt.timestamp)) {
      segMap.set(pt.timestamp, {
    ...
    if (!pt.interpolated) {
      segMap.get(pt.timestamp).interpolated = false;
    }
  }
  ...
  for (const seg of segments) {
    ...
    const gap = new Date(seg.timestamp).getTime() - new Date(last.endTimestamp).getTime();
    if (gap <= 1000) {
      last.endTimestamp = seg.endTimestamp;
      last.duration = Math.round((new Date(last.endTimestamp) - new Date(last.timestamp)) / 1000);
      if (!seg.interpolated) last.interpolated = false;
    } else {
      merged.push({ ...seg });
    }
  }
...
function generate_nightscout_treatmentsv3(batch, timestampDelta) {
  ...
  */
  ...
  if (hypoModes) {
    mergeHypoSegments(hypoModes).forEach(function(element) {
      var treatment = {};
