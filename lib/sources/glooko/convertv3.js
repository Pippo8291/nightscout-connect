...
function generate_nightscout_pumpSettingsv3(batch, timestampDelta) {
  ...
  function generateStore(pumpSettingElement) {

    var f_date = moment(pumpSettingElement.syncTimestamp);
    return {
      [new Date(f_date + timestampDelta).toISOString( )]: {
        dia: pumpSettingElement.generalSettings.activeInsulinTime,
        carbratio: [
          pumpSettingElement.profilesBolus[0].insulinToCarbRatioSegments.data.map(segment => {
        ...
        delay: 12,
        sens: [
          pumpSettingElement.profilesBolus[0].isfSegments.data.map(segment => {
            const seconds = segment.segmentStart * 3600;
            return {time: convertSeconds(seconds),
              timeAsSeconds: seconds,
              value: segment.value}
  ...
  pumpSettings.forEach(function(element) {
    var pumpSetting = {};

    var f_date = moment(element.syncTimestamp);
    pumpSetting.defaultProfile = new Date(f_date + timestampDelta).toISOString( );
    pumpSetting.startDate =  new Date(f_date + timestampDelta).toISOString( );
    pumpSetting.millis =  new Date(f_date + timestampDelta).getTime();
    pumpSetting.created_at = new Date(f_date + timestampDelta).toISOString( );
    pumpSetting.store = generateStore(element);
