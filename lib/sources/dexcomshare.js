const dexcomSessionManager = require('./dexcom_session_manager');
  ...
  var impl = {
    authFromCredentials (creds, settings) {
      ...
      return http.post(modDefaults.auth, body, { params, headers }).then((response) => {
        ...
        console.log("finish1", session)
        dexcomSessionManager.initSession(session);
        console.log("finish")
        return session;
      })
      .catch((err) => {
    ...
    dataFromSesssion(session, last_known) {
      ...
      var body = { };
      var params = {
        sessionID: dexcomSessionManager?.getSession(),
        minutes,
        maxCount,
      };
