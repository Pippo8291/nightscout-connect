    ...
    return http.get('/api/v1/entries.json', {
      ...
      data.forEach(function (entry) {
        ...
        if (closest) {
          ...
          // Start from the existing doc so all its fields are kept as-is.
          // Then fill in only fields that are absent (null or undefined) in
          // the existing doc from the incoming Glooko entry.
          // POST with _id set triggers an upsert in NS v1 (no PUT support).
          var merged = Object.assign({}, closest);
          merged._id = extractOid(closest._id);
          Object.keys(entry).forEach(function (k) {
            if (merged[k] == null) merged[k] = entry[k];
          });
          updatePromises.push(
            http.post('/api/v1/entries.json', [merged], { headers })
    ...
    return http.get('/api/v1/treatments.json', {
      ...
      data.forEach(function (treatment) {
        ...
        if (existingDoc) {
          ...
          // NS v1 has no PUT /treatments/:id. POST with _id set triggers
          // an upsert (Nightscout calls collection.save which replaces by _id).
          // Normalise _id to a plain string so the upsert key is recognised.
          merged._id = extractOid(existingDoc._id);
          updatePromises.push(
            http.post('/api/v1/treatments.json', merged, { headers })
