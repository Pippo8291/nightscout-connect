...
function nightscoutRestAPI (config, axios) {
  ...
  // Nightscout caps how many documents one query returns, so a batch covering
  // a wider window than that cap has to be paged through.  Asking for a single
  // capped page makes every entry beyond the cap look "new", which re-inserts
  // it on every cycle.
  var EXISTING_PAGE_SIZE = 1000;
  var EXISTING_MAX_PAGES = 100;

  /**
   * Collect every entry Nightscout already has between minTime and maxTime,
   * paging downwards by date.  Rejects if any page fails: callers must not
   * treat an incomplete picture as "nothing exists yet".
   */
  function fetch_existing_entries (minTime, maxTime, headers) {
    var collected = [];
    var seen = new Set();

    function page (upperBound, pagesLeft) {
      return http.get('/api/v1/entries.json', {
        headers,
        params: { 'find[date][$gte]': minTime, 'find[date][$lte]': upperBound, count: EXISTING_PAGE_SIZE }
      }).then(function (resp) {
        var docs = resp.data || [];
        var added = 0;
        var oldest = null;
        ...
        docs.forEach(function (doc) {
          if (oldest === null || doc.date < oldest) oldest = doc.date;
          var id = extractOid(doc._id);
        ...
        // A short page means the window is exhausted.  `added === 0` guards
        // against a server that ignores the narrowed bound, so we can never
        // spin here.
        if (docs.length < EXISTING_PAGE_SIZE || added === 0 || pagesLeft <= 1) {
          return collected;
        }

        // The bound is inclusive, so the boundary document comes back once
        // more and is filtered out by `seen` above.
        return page(oldest, pagesLeft - 1);
      });
    }

    return page(maxTime, EXISTING_MAX_PAGES);
  }
  ...
  function record_glucose (data) {
    ...
    return fetch_existing_entries(minTime, maxTime, headers).catch(function (err) {
      // Without a complete picture of what is already stored we cannot tell
      // new readings from known ones.  Inserting anyway is what produced
      // duplicates, so skip this batch; the bookmark stays put and the same
      // window is retried on the next cycle.
      console.log("SKIPPING ENTRIES, existing-entry lookup failed:", err && err.message);
      return null;
  ...
  function bookmark_glucose (data) {
    ...
    if (readings && readings.length) {
      // readings[0] is whatever the batch happened to start with — for the
      // Glooko transform that is the OLDEST reading, which pins the fetch
      // window open forever.  Track the newest instead, and never move the
      // bookmark backwards.
      var newest = null;
      readings.forEach(function (reading) {
        var ms = new Date(reading.dateString || reading.date).getTime();
        if (isNaN(ms)) return;
        if (newest === null || ms > newest) newest = ms;
      });
      if (newest !== null && (!bookmark.entries || newest > bookmark.entries.getTime())) {
        bookmark.entries = new Date(newest);
      }
    }
