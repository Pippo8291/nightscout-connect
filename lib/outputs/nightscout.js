    ...
    return http.get('/api/v1/entries.json', {
      ...
      data.forEach(function (entry) {
        ...
        if (closest) {
          ...
          // Fill in only fields that are absent (null/undefined) from Glooko.
          // NS v1 POST is insert-only, so DELETE then re-POST without _id.
          ...
          updatePromises.push(
            http.delete('/api/v1/entries/' + oid, { headers })
              .then(function () {
                return http.post('/api/v1/entries.json', [merged], { headers });
              })
    ...
    return http.get('/api/v1/treatments.json', {
      ...
      data.forEach(function (treatment) {
        ...
        if (existingDoc) {
          ...
          updatePromises.push(
            http.delete('/api/v1/treatments/' + oid, { headers })
              .then(function () {
                return http.post('/api/v1/treatments.json', [merged], { headers });
              })
